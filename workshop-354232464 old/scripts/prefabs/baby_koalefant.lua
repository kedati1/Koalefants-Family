local function MarkKoalefantBaby(dt)
	local assets = {
		Asset("ANIM", "anim/baby_koalefant_"..dt.name..".zip"),
	...
	local function SetFullyGrown(inst)
		local grown = SpawnPrefab("koalefant_"..dt.name)
	...
	local function onkilledbyother(inst, attacker)
		if GetPlayer() and GetPlayer().components.kramped then
			GetPlayer().components.kramped:OnNaughtyAction(10)
		end
	end
	...
	local function fn(Sim)
		...
		inst.AnimState:SetBuild("baby_koalefant_"..dt.name)
	...
	return Prefab( "common/monsters/baby_koalefant_"..dt.name, fn, assets, prefabs) 
end
...
for k,v in pairs(data) do
	table.insert(ret, MarkKoalefantBaby(v))
end
